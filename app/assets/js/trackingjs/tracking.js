...
(function(window, undefined) {
  ...
  tracking.Frames = {};
  tracking.Frames.defaultFrameDelay = 20;
  tracking.Frames.frameDelayCount = 20;

    /**
     * To set the frame-wise delay to catch the camera frame to include in the tracking canvas.
     *
     * @param frameCount
     */
  tracking.Frames.setFrameDelay = function(frameCount) {
      if((typeof frameCount === undefined) || (frameCount == null) || (isNaN(frameCount))) {
          tracking.Frames.frameDelayCount = parseInt(tracking.Frames.defaultFrameDelay);
      } else {
          tracking.Frames.frameDelayCount = (parseInt(frameCount) < 0) ? parseInt(tracking.Frames.defaultFrameDelay) : parseInt(frameCount);
      }
  };
  ...
  tracking.trackVideo_ = function(element, tracker) {
    ...
    var requestId;
    var requestAnimationFrame_ = function(freq) {
		
	  var cur_freq = parseInt(freq);
      ...
      requestId = window.requestAnimationFrame(function() {
		  if(cur_freq == 0) {
			  if (element.readyState === element.HAVE_ENOUGH_DATA) {
				...
				cur_freq = parseInt(tracking.Frames.frameDelayCount) + 1;
				
		  }
		  
		  cur_freq = cur_freq - 1;
        
        requestAnimationFrame_(cur_freq);
      });
    };
    ...
    task.on('run', function() {
      requestAnimationFrame_(tracking.Frames.frameDelayCount);
    });
    return task.run();
  };
